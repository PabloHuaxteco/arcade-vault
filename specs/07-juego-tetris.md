...
- `app/globals.css`: nueva regla `.game-canvas-tetris` (aspect-ratio `1 / 2`, resto de propiedades iguales a `.game-canvas`: ancho `100%`, `height: auto`, `display: block`, `image-rendering: pixelated`, fondo negro) más una regla mínima para el segundo `<canvas>` de vista previa (tamaño fijo pequeño, mismo borde/fondo que el resto del panel). No se toca `.game-canvas` ni su `aspect-ratio: 4 / 3`.
...
- **Sí:** clase CSS específica `.game-canvas-tetris` con `aspect-ratio: 1 / 2`, dejando `.game-canvas` (`4 / 3`) intacta para asteroides y futuros juegos con ese ratio. Evita recorte por letterboxing y no toca una regla compartida.
- **No:** forzar el canvas de Tetris al `aspect-ratio: 4 / 3` existente. El recorte resultante (bandas o distorsión) no vale la pena frente a una regla CSS nueva y aislada.
